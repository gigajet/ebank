...
public class twoqueue_counsellor extends AppCompatActivity {

    FirebaseDatabase fb;
    TextView tvResult, tvToBeTransmit;
    Button btnGet;
    String TAG="TWOQUEUE-COUNSELLOR";
    String other_handle;

    public void result() {
        twoqueue_counsellor.this.runOnUiThread(new Runnable() {
            @Override
            public void run() {
                ...
                dbref.runTransaction(new Transaction.Handler() {
                    ...
                    public Transaction.Result doTransaction(@NonNull @NotNull MutableData currentData) {
                        ...
                        if (nrootchild == 0) { //first time so null (we have data_leg so child never be 0)
                            ...
                            for (MutableData data : currentData.child("waiting_clients").getChildren()) {
